...
cd $TMP/mods-enabled 2> /dev/null
sudo ln -s /etc/apache2/mods-available/h264_streaming.conf
sudo ln -s /etc/apache2/mods-available/h264_streaming.load

while read line; do sudo a2enmod $line; done < /tmp/vault/a2enmod_enabing_mods.txt


cd /etc/php5/apache2 2> /dev/null
sudo mv $TMP/php.ini "$TMP/orig_$( date | sed 's/ //g')_php.ini.txt"
