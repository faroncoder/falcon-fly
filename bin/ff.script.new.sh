...
if [ -z "$1" ]; then
		...
		logger "`basename $0` : DEBUG: Error 1 - name of script required"
		exit 1;
fi
if [ -z "$2" ]; then
		...
		logger "`basename $0` : DEBUG: Error 2 - name of folder required"
		exit 2;
fi
...
echo -e "$Fstat $Fgreen $proofreadfile $Foff created."
if [[ "$( hostname -s )" == "f10" ]]; then
	EDITOR="subl"
else
	EDITOR="nano"
fi
$EDITOR "$newbin/$proofreadfile" 2> /dev/null
################### END
