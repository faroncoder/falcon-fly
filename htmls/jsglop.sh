...
if [[ -z "$1" ]]; then
		#PATHDIRHOME="/home/faron/.falcon/scripting/falcon-fly/htmls/VAULT/lab/mediaelement"
		PATHDIRHOME="$PWD"
	else
		PATHDIRHOME="$1"
fi
...
function getBootstrap() {
	if [ ! -d "$PATHDIRHOME/jsc/js/custom" ]; then
				...
				cp "/home/faron/.falcon/scripting/falcon-fly/htmls/VAULT/bootstrap.min.js.txt" "$PATHDIRHOME/jsc/js/custom/bootstrap.min.js"
...
FILEPOINT="$PATHDIRHOME/jsc/js/jsengine.js"
> $FILEPOINT
if [ -d "$PATHDIRHOME/jsc/js" ]; then
		echo "App structures match!"
		JSGLOPPER=( $( find -L $PATHDIRHOME/jsc/js -maxdepth 1 -type f -name '*.js' ! -name 'jsengine.js' ! -name '*ie8*' ! -name '*ie9*' ! -name '*ie7*' ! -name '*ie6*' ! -name '*.swf*' ! -name '*silver*xap'  -exec basename {} \; ) )
	 	...
	 	for w in "${JSGLOPPER[@]}";
	 		...
	 		#	sed -i -e 's/"//g' $w
				...
				if [[ "$FILEMIN" = "min" ]]; then
					...
					FILEGRAB="`echo $( rev <<< $w | cut -d '.' -f3 | rev )`"
					FILEID="$FILEGRAB.$FILEMIN.$FILEEXT"
				...
				echo "\$.getScript(\"./jsc/js/$FILEID\", function(data, textStatus) { console.log(\"$w loaded\", textStatus); });" >>  $FILEPOINT
