 ...
 * http://codecanyon.net/item/lightbox-evolution/115655?ref=aeroalquimia
 *
...
(function ($, A, B, C) {
	...
	var E = false;
	var F = (function () {
		for (var a = 3, b = B.createElement("b"), c = b.all || []; b.innerHTML =
	...
	$.extend({
		LightBoxObject: {
			...
			overlay: {
				create: function (a) {
					this.options = a;
					this.element = $('<div id="' + new Date().getTime() + '" class="' + this
						.options.name + '-overlay"></div>');
				...
				show: function (a) {
					...
					this.resize();
					this.hidden = false;
					this.transition = this.element.fadeTo(this.options.showDuration, this.options
						.style.opacity, $.proxy(function () {
				...
				hide: function (a) {
					...
					this.hidden = true;
					this.transition = this.element.fadeTo(this.options.closeDuration, 0, $.proxy(
						function () {
			...
			create: function (a) {
				...
				var c = $('<div class="' + b + ' ' + b + '-mode-image"><div class="' + b +
					'-border-top-left"></div><div class="' + b +
					'-border-top-middle"></div><div class="' + b +
					'-border-top-right"></div><a class="' + b +
					'-button-close" href="#close"><span>Close</span></a><div class="' + b +
					'-navigator"><a class="' + b +
					'-button-left" href="#"><span>Previous</span></a><a class="' + b +
					'-button-right" href="#"><span>Next</span></a></div><div class="' + b +
					'-buttons"><div class="' + b + '-buttons-init"></div><a class="' + b +
					'-button-left" href="#"><span>Previous</span></a><a class="' + b +
					'-button-max" href="#"><span>Maximize</span></a><div class="' + b +
					'-buttons-custom"></div><a class="' + b +
					'-button-right" href="#"><span>Next</span></a><div class="' + b +
					'-buttons-end"></div></div><div class="' + b +
					'-background"></div><div class="' + b + '-html"></div><div class="' + b +
					'-border-bottom-left"></div><div class="' + b +
					'-border-bottom-middle"></div><div class="' + b +
					'-border-bottom-right"></div></div>');
				...
				this.overlay.create({
					...
					showDuration: (E ? this.options.animation.show.duration / 2 : this.options
						.animation.show.duration),
					closeDuration: (E ? this.options.animation.close.duration / 2 : this.options
			...
			ex: function (f, g, h) {
				...
				$.each(f, this.proxy(function (c, d) {
					$.each(d, this.proxy(function (i, e) {
						if ((c == "flash" && g.split('?')[0].match(e.reg)) || (c ==
							"iframe" && g.match(e.reg))) {
							j.href = g;
							if (e.split) {
								var a = c == "flash" ? g.split(e.split)[e.index].split('?')[0].split(
									'&')[0] : g.split(e.split)[e.index];
								j.href = e.url.replace("%id%", a).replace("%width%", h.width).replace(
									"%height%", h.height)
							}
							...
							if (h.width) {
								...
								var b = this.calculate(this.options[j.type].width, this.options[j
									.type].height);
								j.width = b.width;
								j.height = b.height
							}
			...
			custombuttons: function (c, d) {
				...
				$.each(c, this.proxy(function (i, a) {
					var b = $('<a href="#" class="' + a['class'] + '">' + a['html'] +
						'</a>');
			...
			show: function (d, f, g) {
				...
				if (f.width && ("" + f.width).indexOf("p") > 0) {
					f.width = Math.round((m.x - 20) * f.width.substring(0, f.width.indexOf(
						"p")) / 100)
				}
				if (f.height && ("" + f.height).indexOf("p") > 0) {
					f.height = Math.round((m.y - 20) * f.height.substring(0, f.height.indexOf(
						"p")) / 100)
				}
				...
				if (this.utils.isEmpty(f.force) === false) {
					...
					$(l.image).load(this.proxy(function () {
						...
						if (f.width) {
							...
							if (f.maximized || (!f.maximized && a.width != n && a.height !=
								height)) {
								l.buttons.div.show();
								l.buttons.max.removeClass(this.options.name + '-hide');
								l.buttons.max.addClass(this.options.name + (f.maximized ?
									'-button-min' : '-button-max'))
							}
						}
						l.title = (this.utils.isEmpty(f.title)) ? false : $('<div class="' +
							this.options.name + '-title"></div>').html(f.title);
						this.loadimage();
						this.resize(n, height)
					}));
					this.esqueleto.image.onerror = this.proxy(function () {
						this.error(
							"The requested image cannot be loaded. Please try again later.")
					});
					...
					if (f.width) {
						...
						this.error(
							"You have to specify the size. Add ?lightbox[width]=600&lightbox[height]=400&lighbox[iframe]=true at the end of the url."
						);
						return false
					}
					var t = '<iframe id="IF_' + (new Date().getTime()) +
						'" frameborder="0" src="' + k +
				...
				b.bind('complete', this.proxy(function () {
					...
					if (!G.opacity) {
						...
						$(a.image).stop().css("opacity", 0).animate({
							"opacity": 1
						}, (E ? this.options.animation.show.duration / 2 : this.options.animation
			...
			morph: function (d, f, g, h, i) {
				...
				if (j[0] == 1 && j[1] < 8) {
					var k = $.speed({
						queue: i || false,
						duration: (E ? this.options.animation[g].duration / 2 : this.options.animation[
							g].duration),
					...
					return d[k.queue === false ? "each" : "queue"](function () {
						...
						for (var p in f) {
							name = p;
							c.animatedProperties[name] = c.specialEasing && c.specialEasing[name] ||
								c.easing || 'swing'
						}
						$.each(f, function (a, b) {
			...
			calculate: function (x, y) {
				var a = this.options.maxsize.width > 0 ? this.options.maxsize.width :
					this.win.width() - 50;
				var b = this.options.maxsize.height > 0 ? this.options.maxsize.height :
					this.win.height() - 50;
				if (x > a) {
					y = y * (a / x);
					x = a;
			...
			maximinimize: function () {
				...
				var e = {};
				b.max.removeClass(d + '-button-min ' + d + '-button-max').addClass((a.maximized) ?
					d + '-button-max' : d + '-button-min');
				a.loading();
				a.loadimage();
				b.div.show();
				if (a.maximized) {
			...
			link: function (b, c) {
				...
				if (c.gallery) {
					...
					var l = [];
					var m = false;
					$("a[" + g + "], area[" + g + "]", this.ownerDocument).filter("[" + g +
						"=\"" + f + "\"]").each($.proxy(function (i, a) {
						if (d[0] === a) {
			...
			utils: {
				isEmpty: function (a) {
					...
					if (Object.prototype.toString.call(a) === '[object String]' || $.type(a) ===
						"array") return a.length === 0
				}
			}
		},
