...
source /usr/local/bin/.bin/file-name-cleaner
mv *.mkv mkv/

## function to collect all exisiting files in this directory only but one by one at a time

INPUTZ="$( source /usr/bin//find  . -maxdepth 1 -type f  -name '*.avi' ! -name '*.mkv'  ! -name '*.sh'  ! -name '*.txt' | head -n 1 )"
...
source /usr/bin//avidemux "$INPUT" --rebuild-index  --audio-map  --video-map --output-format MATROSKA  --save
