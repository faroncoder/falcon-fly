...
startFFMPEG() {
	for x in "${FILEGRAB[@]}"; do
		...
		OUTPUT="$STEPFULL.mp4"	      
							...
							ffmpeg -i "$STEPFULL" -codec copy -metadata title="$TITLEFILE" -metadata album="$ALBUMFILE" -metadata year="$YEARFILE" -metadata container="$CONTAINTERFILE" -metadata artist="$ARTISTFILE" -metadata comment="$COMMENTFILE" -f mp4  "mp4/$OUTPUT" < /dev/null
