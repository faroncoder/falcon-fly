...
if [[ $EUID == 0 ]]; then
	...
	ALBUMFILE="$HOST @ $IP"
	TITLEFILE="$NEWFILE | $THISVERSION"
	ARTISTFILE="$USER @$HOST <--$IP"
	CODECVID="libx264"
	...
	#	chmod 555 $INFOA
		...
		ffmpeg -i $GETFILE -y -codec:v $CODECVID -preset $CALCULATED -maxrate "$( echo $MAXRAT)k" \
...
#		chmod 555 $COLLECT	
	...
	if [[ "$GETFILE" == "" ]]; then
		GETFILE=( $( find $PWD -maxdepth 1 -type f ) )
	fi
