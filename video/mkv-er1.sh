...
/home/faron/bin/core/scripting/faron-scriptor/bin/file-name-cleaner.sh
mv *.mkv mkv/

## function to collect all exisiting files in this directory only but one by one at a time

INPUTZ="$(  /usr/bin/find  . -maxdepth 1 -type f  -name '*.avi' ! -name '*.mkv'  ! -name '*.sh'  ! -name '*.txt' )"
...
/usr/bin/avidemux "$INPUT" --rebuild-index  --audio-map  --video-map --output-format MATROSKA  --save "mkv/$PREFILE.mkv"
