...
INPUT="$( find $PWD -maxdepth 1 -type f -name '*.mp4' -exec basename {} \; | sort | head -n 1)"
if [ -z "$1" ];
	then
...
for f in "${INPUT[@]}"
	do
		...
		ffmpeg -ss 00:01:00 -i $f -y -t 1 -vf scale=-1:$HEIGHTPNG -f image2 -vframes 1 "thumbs/$f.png" < /dev/null ;
